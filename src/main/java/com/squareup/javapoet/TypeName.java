 ...
 * identifier for primitive
 * types like {@code int} and raw reference types like {@code String} and
 * {@code List}. It also
 ...
 * For example, the
 * type name for {@code java.util.List} doesn't know about the {@code size()}
 ...
 * ClassName#get(String, String, String...)}. To build composite types like
 * {@code char[]} and
 * {@code Set<Long>}, use the factory methods on {@link ArrayTypeName}, {@link
 * ParameterizedTypeName}, {@link TypeVariableName}, and
...
public class TypeName {
   ...
   *
   * @throws UnsupportedOperationException if this type isn't eligible for
   *                                       unboxing.
   */
  ...
  CodeWriter emitLeafType(CodeWriter writer) throws IOException {
    ArrayTypeName arrayRepresentation = TypeName.asArray(this);
    if (arrayRepresentation != null) {
      TypeName elementType = arrayRepresentation.componentType;
      if (elementType != null) {
        // Handle variable arguments by outputting the element type without the array
        // notation
        return elementType.emitLeafType(writer);
      }
    }
    return this.emit(writer);
  }
}
